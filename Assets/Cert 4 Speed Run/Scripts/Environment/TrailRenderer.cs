...
namespace CertIVSpeedrun.Environment
{
  public class TrailRenderer : MonoBehaviour
  {
    ...
    void Update()
    {
      ...
      if(points.Count > 1)
      {
        Vector3 cur1 = UnityEngine.Camera.main.WorldToScreenPoint(((Point) points[0]).position);
        lastCameraPosition1.z = 0;
        Vector3 cur2 = UnityEngine.Camera.main.WorldToScreenPoint(((Point) points[points.Count - 1]).position);
      ...
      if(re)
      {
        ...
        if(points.Count > 1)
        {
          ...
          foreach(Point p in points)
          {
            ...
            Vector3 vectorToCamera = UnityEngine.Camera.main.transform.position - p.position;
